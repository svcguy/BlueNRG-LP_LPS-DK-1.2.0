...
/**
 * @brief The device is waiting events.
 * Device can go to sleep.
 */
#define HID_DEVICE_READY_TO_SLEEP         0x01

/**
 * @brief The deivce is ready to send Notification report to
 * the HID Host
 */
#define HID_DEVICE_READY_TO_NOTIFY        0x02

/**
 * @brief The device is Bonded
 */
#define HID_DEVICE_BONDED                 0x04

/**
 * @brief The device is Connected
 */
#define HID_DEVICE_CONNECTED              0x08

/**
 * @brief The device is discoverable upon user action
 */
#define HID_DEVICE_DISCOVERABLE          0x10

/**
 * @brief The device is reconnecting before it is bonded and a notification
 * needs to be sent.
 */
#define HID_DEVICE_RECONNECTING          0x20

//@} \\END HID/HOGP Device Status constants
   
#define GENERAL_DISCOVERY_FLAGS (FLAG_BIT_LE_GENERAL_DISCOVERABLE_MODE|FLAG_BIT_BR_EDR_NOT_SUPPORTED)
#define LIMITED_DISCOVERY_FLAGS (FLAG_BIT_LE_LIMITED_DISCOVERABLE_MODE|FLAG_BIT_BR_EDR_NOT_SUPPORTED)
#define DISCOVER_FLAGS_INDEX 2


#define ADV_AD_TYPE_FLAG_LEN 3
#define ADV_SERV_UUID_LEN 4
#define ADV_LOCAL_NAME_MAX_LEN (ADV_MAX_LEN - ADV_AD_TYPE_FLAG_LEN - ADV_SERV_UUID_LEN -2)
#define ADV_LOCAL_NAME_INDEX (ADV_AD_TYPE_FLAG_LEN + ADV_SERV_UUID_LEN + 2)   
...
uint8_t hidDevice_Init(uint8_t IO_Capability, connParam_Type connParam, 
                       uint8_t dev_name_len, uint8_t *dev_name, uint8_t identity_address_type);
 ...
 *
 * This function makes use of malloc(), so an heap memory is required.
 * @param battery Battery Service characteristics to add. 
 * See the typedef batteryService_Type for more details.
...
/**
 * @brief Veirfies if the Input Report is ready/enabled to send data
 ...
 * - HID_DEVICE_READY_TO_SLEEP : state machine is waiting events. Device can go to sleep
 * - HID_DEVICE_READY_TO_NOTIFY : device connected and paired
 * - HID_DEVICE_BONDED : device bonded
 * - HID_DEVICE_CONNECTED : device connected
 * - HID_DEVICE_DISCOVERABLE : device discoverable
 * - HID_DEVICE_RECONNECTING : device reconnecting to send notifications 
 */
...
/******* HID/HOGP Event Callback Prototypes *******/
...
void HID_Lib_aci_gatt_srv_attribute_modified_event(uint16_t Connection_Handle,
...
void HID_Lib_HAL_VTimerTimeoutCallback(void *param);

void hidSetTimerCallBack(void);
