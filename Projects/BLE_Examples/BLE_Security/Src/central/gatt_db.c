...
static securitySetType sec_param;
...
{   
    ...
    if (masterContext.findCharacOfService) {
      ...
      timeRXChar = HAL_VTIMER_GetCurrentSysTime();
    }
   ...
   if (masterContext.read_char ==TRUE)
   {
      ...
      if ((HAL_VTIMER_DiffSysTimeMs(currentTime, timeRXChar)) >= RX_CHAR_TIMEOUT) 
      {
        timeRXChar = HAL_VTIMER_GetCurrentSysTime();
...
{  
  if (masterContext.start_insert_passkey || masterContext.do_terminate_connection || masterContext.start_confirm_numeric_value || BSP_COM_TxFifoNotEmpty())
  {
     /* In order to be able to insert the Pass key through keyboard sets power save level to 
...
uint8_t deviceInit(void)
{
  ...
  param.txPowerLevel = 25;
  param.Identity_Address_Type = IDENTITY_ADDRESS; 
...
void findCharcOfService(void)
{
  ...
  {
    PRINTF("Master_GetCharacOfService() OK.\r\n");
  }
...
void Master_ServiceCharacPeerDiscovery_CB(uint8_t *procedure, uint8_t *status, uint16_t *connection_handle)
{
  ...
  case GET_CHARACTERISTICS_OF_A_SERVICE:
    {
      //PRINTF("Master_ServiceCharacPeerDiscovery_CB() *procedure: 0x%02x, *status: 0x%02x\r\n", *procedure, *status);
      if (*status == BLE_STATUS_SUCCESS) {
