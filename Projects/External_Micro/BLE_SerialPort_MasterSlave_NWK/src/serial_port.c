...
static uint8_t adv_data[] = {0x02,AD_TYPE_FLAGS, FLAG_BIT_LE_GENERAL_DISCOVERABLE_MODE|FLAG_BIT_BR_EDR_NOT_SUPPORTED,
                          16, AD_TYPE_COMPLETE_LOCAL_NAME,'S','P','o','r','t','_','L','P','_','M','S'};
...
uint8_t Find_DeviceName(uint8_t data_length, uint8_t *data_value)
{
  ...
  while (index < data_length) {
    ...
    /* Check if field is complete local name and the lenght is the expected onet  */
...
* Description    : Init SerialPort device.
* Input          : None.
* Return         : Status.
*******************************************************************************/
uint8_t SerialPort_DeviceInit(void)
{
  ...
  /* GAP Init */
  ret = aci_gap_init(GAP_CENTRAL_ROLE|GAP_PERIPHERAL_ROLE,0x0,0x07, 0, &service_handle,
...
void APP_Tick(void)
{
  ...
  if (device_role == MASTER_ROLE) {
    ...
    if(APP_FLAG(CONNECTED) && APP_FLAG(END_READ_TX_CHAR_HANDLE) && APP_FLAG(END_READ_RX_CHAR_HANDLE) && !APP_FLAG(NOTIFICATIONS_ENABLED)) {
      ...
      if(aci_gatt_clt_write_nwk(connection_handle, tx_handle + 2,
                             sizeof(client_char_conf_data),
                             client_char_conf_data)==BLE_STATUS_SUCCESS) { 
