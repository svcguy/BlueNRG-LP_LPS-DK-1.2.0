...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\HAL\\UART\\UART_HwFlowControl\\WiSE-Studio\\{STEVAL-IDB011V1|STEVAL-IDB012V1}</tt> 
  -# Select desired configuration to build
...
|     B2     |       USART RTS     |      USART RTS     |
|     B3     |       USART CTS     |      USART CTS     |
|     B4     |       Not Used      |        DL2         |
...
This later is calling the HAL_UART_MspInit()function which core is implementing the configuration of the needed UART resources according to the used hardware (CLOCK, GPIO and NVIC). You may update this function to change UART configuration.

The UART/Hyperterminal communication is then initiated.
The HAL_UART_Receive_IT() and the HAL_UART_Transmit_IT() functions allow respectively the reception of Data from Hyperterminal and the transmission of a predefined data buffer  with hardware flow control.
...
For this example the TxBuffer (aTxStartMessage) is predefined and the RxBuffer (aRxBuffer) size is limited to 10 data by the mean of the RXBUFFERSIZE define in the UART_HwFlowControl_main.c file.
...
uint8_t aTxStartMessage[] = "\n\r ****UART-Hyperterminal communication based on IT using HFC****\n\r Enter 10 characters using keyboard :\n\r";
