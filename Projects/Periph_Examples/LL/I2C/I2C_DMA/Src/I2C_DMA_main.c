 ...
 * @brief How to transmit data bytes from an I2C master device using DMA mode to an I2C slave 
...
* \section KEIL_project KEIL project
  ...
  -# Open the KEIL project
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\I2C\\I2C_DMA\\MDK-ARM\\{STEVAL-IDB011V1|STEVAL-IDB012V1}\\I2C_DMA.uvprojx</tt>
...
* \section IAR_project IAR project
  ...
  -# Open the IAR project
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\I2C\\I2C_DMA\\EWARM\\{STEVAL-IDB011V1|STEVAL-IDB012V1}\\I2C_DMA.eww</tt>
  -# Select desired configuration to build
...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\I2C\\I2C_DMA\\WiSE-Studio\\{STEVAL-IDB011V1|STEVAL-IDB012V1}</tt> 
  -# Select desired configuration to build
...
|     DL2    |   ON: Data reception is OK - Fast blinking: wait for user-button press - Slow blinking: error  |   ON: Data reception is OK - Fast blinking: wait for user-button press - Slow blinking: error  |   ON: Data reception is OK - Fast blinking: wait for user-button press - Slow blinking: error  |   ON: Data reception is OK - Slow blinking: error  |   ON: Data reception is OK - Slow blinking: error  |   ON: Data reception is OK - Slow blinking: error  |
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
|      PUSH1     |  Start of the communication by Master  |  Start of the communication by Master  |  Start of the communication by Master  |  Start of the communication by Master  |  Start of the communication by Master  |  Start of the communication by Master  |
...
How to transmit data bytes from an I2C master device using DMA mode to an I2C slave device using DMA mode. 
The peripheral is initialized with LL unitary service functions to optimize for performance and size.

This example guides you through the different configuration steps by mean of LL API to configure GPIO, DMA and I2C peripherals using two BLUENRG_LP.
...
The user can choose between Master and Slave through "#define SLAVE_BOARD" in the "I2C_DMA_main.h" file:
...
When acknowledge is received on I2C2 (Master), DMA transfer the data from embedded SRAM buffer to I2C2 TXDR register (Master).
...
DMA transfer the next data from embedded SRAM buffer to I2C2 TXDR register (Master) until Transfer completed.
...
*/
__IO uint8_t  ubNbDataToTransmit             = sizeof(aLedOn)-1;
...
static void MX_I2Cx_Init(void)
{
  ...
  /* I2Cx_RX Init */
  LL_DMA_SetPeriphRequest(DMA1, LL_DMA_CHANNEL_3, LL_DMAMUX_REQ_I2Cx_RX);
  ...
  /* I2Cx_TX Init */
  LL_DMA_SetPeriphRequest(DMA1, LL_DMA_CHANNEL_2, LL_DMAMUX_REQ_I2Cx_TX);
  ...
  LL_DMA_ConfigAddresses(DMA1, LL_DMA_CHANNEL_3, (uint32_t)LL_I2C_DMA_GetRegAddr(I2Cx, LL_I2C_DMA_REG_DATA_RECEIVE), (uint32_t)&(aReceiveBuffer), LL_DMA_GetDataTransferDirection(DMA1, LL_DMA_CHANNEL_3));
  LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_2, ubNbDataToTransmit);
  LL_DMA_ConfigAddresses(DMA1, LL_DMA_CHANNEL_2, (uint32_t)pTransmitBuffer, (uint32_t)LL_I2C_DMA_GetRegAddr(I2Cx, LL_I2C_DMA_REG_DATA_TRANSMIT), LL_DMA_GetDataTransferDirection(DMA1, LL_DMA_CHANNEL_2));
...
void Handle_I2C_Slave(void)
{
  ...
  /* Loop until end of transfer completed (DMA TC raised) *****************/
  ...
  /* Loop until end of slave reception completed (STOP flag raised) *******/
  ...
  /* Clear pending flags, Data consistency are checking into Slave process */
...
void Handle_I2C_Master(void)
{
  ...
  */
  ...
  /* Loop until end of master transfer completed (STOP flag raised) *******/
