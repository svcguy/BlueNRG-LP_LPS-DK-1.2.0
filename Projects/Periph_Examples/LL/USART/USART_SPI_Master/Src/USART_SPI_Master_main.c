...
* File Name          : USART_SPI_Master_main.c
...
/**
 * @file  USART_SPI_Master/USART_SPI_Master_main.c
...
* \section KEIL_project KEIL project
  ...
  -# Open the KEIL project
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\USART\\USART_SPI_Master\\MDK-ARM\\{STEVAL-IDB011V1|STEVAL-IDB012V1}\\USART_SPI_Master.uvprojx</tt>
...
* \section IAR_project IAR project
  ...
  -# Open the IAR project
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\USART\\USART_SPI_Master\\EWARM\\{STEVAL-IDB011V1|STEVAL-IDB012V1}\\USART_SPI_Master.eww</tt>
  -# Select desired configuration to build
...
* \section WISE_project WiSE-Studio project
  ...
  -# Select File, Import, Existing Projects into Workspace
     <tt>C:\\Users\\{username}\\ST\\BlueNRG-LP_LPS DK x.x.x\\Projects\\Periph_Examples\\LL\\USART\\USART_SPI_Master\\WiSE-Studio\\{STEVAL-IDB011V1|STEVAL-IDB012V1}</tt> 
  -# Select desired configuration to build
...
|     DL3    |   ON: OK - Fast blinking: wait User action - Slow blinking: error  |   ON: OK - Fast blinking: wait User action - Slow blinking: error  |   ON: OK - Fast blinking: wait User action - Slow blinking: error  |
...
This example shows how to configure GPIO and USART peripheral to send characters synchronously to/from the LL SPI SPI_IT_Slave_Init in DMA mode. 
...
Example execution:
After startup from reset and system configuration, LED3 is blinking quickly and wait User push-button (PUSH1) action.
...
In case of both transfers successfully completed, LED3 is turned on.
...
On press on push button , The communication is enabled.
...
To test this SPI Master application was been used the LL/SPI/SPI_IT_Slave_Init with the SPI configured  as SPI1 and the data size set to 8bit.
It is mandatory uncomment to the slave application the follows define in the main.h file:
 #define CONFIG_DATASIZE_8BIT  1
 #define USE_SPI1_PINS         1    // only for BlueNRG-LP

 |   BlueNRG-LP     |    BlueNRG-LP   |    
 |    MASTER        |      SLAVE      |
...
When the SPI slave shows on COM the waiting message for receiving data, press the PUSH1 button of the master. The master application doesn't show meaning messsage on COM. LED3 is turned on in case of success. The master COM is used to virtualize the SPI. The SPI Slave shows on COM port a success message in case of well received message.
...
/* Includes ------------------------------------------------------------------*/
#include "USART_SPI_Master_main.h"
...
void WaitAndCheckEndOfTransfer(void)
{
  ...
  // Release the CS signal after RX is completed to have same behavior of the SPI
  LL_GPIO_SetOutputPin(GPIO_PORT_MASTER_CS, GPIO_PIN_SPI_MASTER_CS);
